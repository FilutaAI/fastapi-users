...
class RefreshTokenManager[RTP]:
    ...
    async def generate_new_token_for_user(self, user: models.UP) -> RTP:
        token = self.generate_refresh_token()

        return await self.refresh_token_db.create({"token": token, "user_id": user.id})

    async def find_refresh_token(self, refresh_token: str, lifetime_seconds: int | None = None) -> RTP | None:
        max_age = None
